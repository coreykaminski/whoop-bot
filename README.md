...
### 9. Deploy on Railway (10am + 10pm Eastern)

This repo includes `scheduler.py`, which runs at **10:00 and 22:00 America/New_York**.
...
3. Add environment variables in Railway:
   - `WHOOP_CLIENT_ID`
   - `WHOOP_CLIENT_SECRET`
   - `WHOOP_REFRESH_TOKEN` (recommended for cloud; generated by `auth_whoop.py`)
   - `ANTHROPIC_API_KEY`
   - `TELEGRAM_BOT_TOKEN`
   - `TELEGRAM_CHAT_ID`
   - Optional: `CLAUDE_MODEL`

To get `WHOOP_REFRESH_TOKEN`, run `python3 auth_whoop.py` locally once and copy the `refresh_token` from `tokens.json` into Railway env vars.
